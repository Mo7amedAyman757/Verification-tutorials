class scoreboard;
    ...
    transaction tr;
    mailbox #(transaction) mb_mon_sb;

    function new(mailbox #(transaction) mb_from_mon);
        this.mb_mon_sb = mb_from_mon;
    endfunction

    function void alu_ref_model (
        input  logic [3:0] a,
        input  logic [3:0] b,
        input  logic       cin,
        input  logic [3:0] ctl,
    ...
    task run();
        ...
        forever begin
            ...
            tr.fail_count++;
            $display("Mismatch in transaction ID: %0d", tr.transaction_id);
            $display("Expected: alu=%0d, carry=%0b, zero=%0b", alu_ref, carry_ref, zero_ref);
            $display("Received: alu=%0d, carry=%0b, zero=%0b", tr.alu, tr.carry, tr.zero);
            end else begin
                $display("Transaction ID: %0d passed.", tr.transaction_id);
                tr.pass_count++;
            end
        end
