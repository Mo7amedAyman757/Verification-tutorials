...
module counter_tb();
    ...
    initial begin
        clk_tb = 0;
        forever begin            
            #5 clk_tb = ~clk_tb; // Generate a clock signal with a period of 10 time units
            stimulus.clk = clk_tb; // Update the clock in the stimulus class
        end
    end
    ...
    initial begin
        ...
        repeat(500) begin
            assert (stimulus.randomize() with {load_n == 1; up_down == 1; ce == 1; rst_n == 1;});
            rst_n_tb = stimulus.rst_n;
            load_n_tb = stimulus.load_n;
            ce_tb = stimulus.ce;   
            up_down_tb = stimulus.up_down;
        ...
        repeat(500) begin
            assert (stimulus.randomize() with {load_n == 1; up_down == 0; ce == 1; rst_n == 1;});
            rst_n_tb = stimulus.rst_n;
            load_n_tb = stimulus.load_n;
            ce_tb = stimulus.ce;   
            up_down_tb = stimulus.up_down;
    ...
    task golden_model(input counter_class stimulus_c);
        if (!stimulus_c.rst_n)
            ...
            if (stimulus_c.up_down)
                ...
                count_out_exp = count_out_exp - 1;
        max_count_exp = (count_out_exp == {stimulus_c.COUNTER_WIDTH{1'b1}})? 1:0;
    ...
    task check_result(input counter_class stimulus_t);
        ...
        @(negedge clk_tb);
        stimulus.count_out = count_out_tb; // Update the count_out in the stimulus class for coverage
